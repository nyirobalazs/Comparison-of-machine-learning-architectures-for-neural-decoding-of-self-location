...
class DataFile:
    ...
    def create_or_update(self, hdf5_file, dataset_name, dataset_shape, dataset_type, dataset_value):
            ...
            if not dataset_name in hdf5_file:
                ...
                if hdf5_file[dataset_name].shape[0]==dataset_shape[0]:
                    new_shape_arr = list(new_shape)
                    new_shape_arr[0]+=dataset_shape[0]
                    new_shape = tuple(new_shape_arr)
    ...
    def write_processed_h5_file(self, path='./', dataset=None):
        ...
        if len(dataset.ft_extracted.shape)==4:
            ...
            self.create_or_update(h5_file, dataset_name="outputs/ft_extracted", dataset_shape=[1,], dataset_type=np.float64, dataset_value=None)
        ...
        h5_file.flush()
        h5_file.close()
        print(print_terminal(type='done',message='Processed dataset writing to .h5 file is done.'))
    ...
    def load_processed_h5(self, path='./'):
        ...
        if self.is_exist(self.source_path):
            ...
            ds.raw_led_positions = raw_data["outputs/raw_led_positions"]
            ds.led_timestamps = raw_data["outputs/led_timestamps"]
            ds.tetrode_timestamps = raw_data["outputs/processed_timestamps"]
...
def save_variables(path='./',
    ...
    variables = {
        'save_datetime' : str(datetime.now().strftime("%d/%m/%Y-%H:%M:%S")),
        ...
        'learn_indexes' : ",".join(np.char.mod('%d',lr_ind)),
        'valid_indexes' : ",".join(np.char.mod('%d',val_ind)),
        'test_indexes'  : ",".join(np.char.mod('%d',ts_ind)),
        'is_shuffle'    : shuffl,
        'filter_param'  : fparam,
...
def data_load_pipeline(path='./', original_srate=None, led_srate=None, is_test=False):
    ...
    if not ('processed' in fname.split("_")):
    ...
    elif ('processed' in fname.split("_")):
