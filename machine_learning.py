...
class CNN_BiLSTM_model:
    ...
    def make_default_hidden_layers(self, input_kernel, num_lstm_units):
        ...
        x = layers.Conv1D(filters = 64, kernel_size = 1, activation = 'relu')(input_kernel)
        x = layers.Dropout(0.3)(x)
        x = layers.Bidirectional(layers.LSTM(num_lstm_units, return_sequences=True))(x)
        x = layers.Dropout(0.3)(x)
    ...
    def build_speed_branch(self, hidden_layer):   
        ...
        x = layers.Dense(1, activation='linear', name="speed_output")(hidden_layer)
        x = layers.Activation("linear", name="speed_output")(x)
        return x
    
    def build_head_dir_branch(self, hidden_layer):   

        x = layers.Dense(1)(hidden_layer)
        x = layers.Activation("linear", name="head_dir_output")(x)
        return x
    ...
    def build_pos_Y_branch(self, hidden_layer):   
        ...
        x = layers.Dense(1)(hidden_layer)
        x = layers.Activation("linear", name="position_Y_output")(x)
        return x    
    
    def assemble_full_model(self, input_shape, nlstm_units=100):
        #input_shape-->(time_steps, nchannels)
        
        input_kernel = layers.Input(shape=input_shape)
        inputs = self.make_default_hidden_layers(input_kernel=input_kernel, num_lstm_units=nlstm_units)
...
class CNN_transferlearn_model:
    ...
    def make_default_hidden_layers(self, input_size):

        model = EfficientNetB6(include_top=False, weights="imagenet")
        config = model.get_config()

        model_custom =  EfficientNetB6(include_top=False,input_shape=input_size, weights=None)
    ...
    def build_speed_branch(self, hidden_layer):   
        ...
        x = layers.Activation("linear", name="speed_output")(hidden_layer)
        
        return x
    
    def build_head_dir_branch(self, hidden_layer):   

        x = layers.Activation("linear", name="head_dir_output")(hidden_layer)
        
        return x
    
    def build_pos_X_branch(self, hidden_layer):   

        x = layers.Activation("linear", name="position_X_output")(hidden_layer)
        
        return x
    
    def build_pos_Y_branch(self, hidden_layer):   

        x = layers.Activation("linear", name="position_Y_output")(hidden_layer)
        
        return x
    
    def assemble_full_model(self, input_shape):

        inputs = self.make_default_hidden_layers(input_shape)
        speed_branch = self.build_speed_branch(inputs)
        head_dir_branch = self.build_head_dir_branch(inputs)
        pos_x_branch = self.build_pos_X_branch(inputs)
        pos_y_branch = self.build_pos_Y_branch(inputs)
...
class Training:
    ...
    def generate_data(self, dataset, data_idx, is_training, stype = 'ft_extr', batch_size=16):
        """
        ...
        while True:
            for idx in data_idx:
                ...
                if len(signals) >= batch_size:
                    yield np.array(signals), [np.array(speeds), np.array(head_dirs), np.array(position_X), np.array(position_Y)]
                    signals, speeds, head_dirs, position_X, position_Y = [], [], [], [], []
    ...
    def train_model(self,dataset, model_name='CNN_behav_cloning',valr=0.3,testr=0.1,shuff=True,col=None,opr=None,val=None, save_path='./models/model.h5'):
        """
        ...
        if model_name=='CNN_transf':
            ...
            model = CNN_BiLSTM_model().assemble_full_model(self.input_shape, nlstm_units=100)
        else:
        ...
        #Set up generators
        train_gen = self.generate_data(dataset, train_idx, is_training=True, stype = signal_types[model_name], batch_size=self.batch_size)
        valid_gen = self.generate_data(dataset, valid_idx, is_training=True, stype = signal_types[model_name], batch_size=self.valid_batch_size)
        ...
        lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=self.init_lr,
                                                                     decay_steps=self.decay_steps, decay_rate=self.init_lr / self.epochs)
...
class Evaluating:
    ...
    def predict(self, model, test_generator, test_idx):
        """
        ...
        for inp in test_generator:
            ...
            pred_speed = speed[0][0]
            pred_head_dir = head_dir[0][0]
            pred_pos_x = pos_x[0][0]
            pred_pos_y = pos_y[0][0]
            
            truth_speed = inp[1][0][0]
            truth_head_dir = inp[1][1][0]
            truth_pos_x = inp[1][2][0]
            truth_pos_y = inp[1][3][0]
    ...
    def eval_pipeline(self,train_obj, test_idx, dataset, sign_type, nelements=100,inp_model=None, model_path=None, save_path='./results/test_resuts.csv'):
        """
        ...
        #2. Generate data
        test_gen = train_obj.generate_data(dataset, test_idx, is_training=False, stype=sign_type, batch_size=1)
        
        #3. Predicting
        pred_df,describe = self.predict(model, test_gen, test_idx)
        print(print_terminal(type='done',message='Evaulation done.'))
        
        #4. Save df
        if self.is_save:
            save_path='{0}results/{1}_results.csv'.format(save_path[0],save_path[1])
            self.save_test_to_csv(save_path,pred_df)
            self.save_test_to_csv(save_path+'',describe)
...
def train_pipeline(dataset, mname, window_size, init_lr=1e-2, epochs=2, batch_size=10,
                   ...
                   testr=0.1, shuff=True, col=None, opr=None, val=None, is_save=True, use_saver=True,save_path='./'):
    """
