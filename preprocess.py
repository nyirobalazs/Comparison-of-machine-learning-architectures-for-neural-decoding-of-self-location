...
class Preprocess():
    ...
    def process_window(self, 
        ...
        Args:
            ...
            channels(None,int,tuple,list, numpy array): the channels which needed to be processed. If None-> proccess all good channel| tuple->(from,to)|int->(0,to)
        ...
        if dim_check:
            ...
            if channels==None:
                good_channels = self.good_ch
            elif type(channels)==list or type(channels).__module__ == np.__name__:
                if all([ch in self.good_ch for ch in channels]):
                    good_channels = np.array(channels)
                else:
                    print(print_terminal(type='error',message='one element of the channels in process_windows is not exist in good_channels'))
            elif type(channels)==tuple:
                good_channels = self.good_ch[channels[0]:channels[1]]
            elif type(channels)==int:
                good_channels = self.good_ch[:channels]
            signal_out = input_signal[:,good_channels]
                ...
                if tasks[taskID] != False:
                    ...
                    if len(signal_out.shape)==2:
                        if not self.cutoff ==0: #if cutoff>0 cut off the first and last few ms of the signall
                            ...
                            signal_out = signal_out[self.cutoff:-self.cutoff,:]
                            timestamps_out = timestamps_out[self.cutoff:-self.cutoff]
                        if win_per_chunk!=None or win_per_chunk!=0:
                            signal_out = np.reshape(signal_out, (win_per_chunk,-1,signal_out.shape[1]))
                            timestamps_out = np.reshape(timestamps_out, (win_per_chunk,-1))
                    elif len(signal_out.shape)==1:
                        if not self.cutoff ==0:
                            ft_extr_out = []
                            signal_out = signal_out[self.cutoff:-self.cutoff]
                            timestamps_out = timestamps_out[self.cutoff:-self.cutoff]
    ...
    def preprocess_chunks(self, dataset, tasks, is_test=False, channels=None,chunk_perc=None, live_write=False, hdf5_file_name=None):
        """
        ...
        if is_test:    
            ...
            print(print_terminal(type='run',message='Test mode is active. Only 1 chunk will be processed.'))
        elif chunk_perc!=None:
            num_ch_proc = int(num_ch_proc*(chunk_perc/100))
            print(print_terminal(type='run',message='Chunk_perc is active. Only {0} chunk will be process from the all {1} chunks.'.format(num_ch_proc,num_chunks)))
        ...
        for chunk_idx in trange(num_ch_proc, desc='Processing chunks'):
            ...
            raw_chunk   = np.array(dataset.raw_signal[chunk_start:chunk_stop,:])
            ...
            if live_write:
                if chunk_idx == 0: 
                    folder= '/'.join(hdf5_file_name.split('/')[:-1])+'/'
                    format= pathlib.Path(hdf5_file_name).suffix
                    if len(format)==0:
                        fname= str(hdf5_file_name.split('/')[-1])
                    else:
                        fname= str(hdf5_file_name.split('/')[-1][:-len(format)])
        ...
        if not live_write:
            ...
            hdf5_file = h5py.File(file_path, mode='r')
...
def preprocess_pipeline(dataset, 
    ...
    if load_code==0:
        ...
        if live_write:
            prep.preprocess_chunks(dataset,tasks, is_test=is_test,channels=channels, live_write=live_write,chunk_perc=chunk_perc, hdf5_file_name=file_path)
        else:
            prep.preprocess_chunks(dataset,tasks, is_test=is_test,channels=channels,chunk_perc=chunk_perc, live_write=live_write)
